...
if [[ $1 == 'tear_down' ]]; then
  ...
  echo "Torn Down UAT Branch - evc-$DRONE_SOURCE_BRANCH.internal.branch.sas-notprod.homeoffice.gov.uk"
  exit 0
fi
...
if [[ ${KUBE_NAMESPACE} == ${BRANCH_ENV} ]]; then
  ...
  echo "STG internal - $APP_NAME.stg.internal.sas-notprod.homeoffice.gov.uk"
  echo "STG external - $APP_NAME.stg.sas.homeoffice.gov.uk"
elif [[ ${KUBE_NAMESPACE} == ${PROD_ENV} ]]; then
